...
void BirdStatistics::printStats() const {
    ...
    if (most_seen > 0) {
        Serial.println("\nMost seen bird ID: " + String(most_seen) + 
                      " (" + String(getEncounterCount(most_seen)) + " times)");
    }
    
    if (rarest > 0) {
        Serial.println("Rarest bird ID: " + String(rarest) + 
                      " (" + String(getEncounterCount(rarest)) + " times)");
    }
