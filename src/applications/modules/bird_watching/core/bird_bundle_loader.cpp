...
BirdBundleLoader::BirdBundleLoader() : is_loaded_(false) {}
...
bool BirdBundleLoader::loadBundle(const std::string& bundle_path)
{
    ...
    is_loaded_ = true;
    LOG_INFO("BUNDLE", "Bundle loaded: " + String(header_.frame_count) + " frames, " + String(header_.frame_width) +
...
bool BirdBundleLoader::loadFrame(uint16_t frame_index, lv_image_dsc_t** out_dsc, uint8_t** out_data)
{
    ...
    if(frame_index >= header_.frame_count) {
        LOG_ERROR("BUNDLE", "Frame index out of range: " + String(frame_index) + "/" + String(header_.frame_count));
        return false;
    }
    ...
    if(file.read((uint8_t*)&header_cf, 4) != 4 || file.read((uint8_t*)&flags, 4) != 4 ||
       file.read((uint8_t*)&width, 2) != 2 || file.read((uint8_t*)&height, 2) != 2 ||
       file.read((uint8_t*)&stride, 4) != 4 || file.read((uint8_t*)&reserved_2, 4) != 4 ||
    ...
    if(color_format != RGB565_COLOR_FORMAT || magic != 0x37) {
        LOG_ERROR("BUNDLE", "Invalid LVGL format in frame " + String(frame_index) + ": cf=0x" +
                                String(color_format, HEX) + ", magic=0x" + String(magic, HEX));
        file.close();
        return false;
    }
    ...
    if(free_heap < data_size + 4096) {
        LOG_ERROR("BUNDLE", "Insufficient memory - need " + String(data_size) + " + 4096, have " + String(free_heap));
        file.close();
        return false;
    }
    ...
    if(bytes_read != data_size) {
        LOG_ERROR("BUNDLE", "Failed to read pixel data: " + String(bytes_read) + "/" + String(data_size));
        free(img_dsc);
        free(img_data);
        return false;
    }
...
bool BirdBundleLoader::validateHeader()
{
    ...
    if(header_.magic != BUNDLE_MAGIC) {
        LOG_ERROR("BUNDLE", "Invalid magic number: 0x" + String(header_.magic, HEX) + " (expected 0x" +
                                String(BUNDLE_MAGIC, HEX) + ")");
        return false;
    }
    ...
    if(header_.version != BUNDLE_VERSION) {
        LOG_WARN("BUNDLE",
                 "Bundle version mismatch: " + String(header_.version) + " (expected " + String(BUNDLE_VERSION) + ")");
        // 版本不匹配只是警告，不阻止加载
    }
    ...
    if(header_.frame_width == 0 || header_.frame_height == 0) {
        LOG_ERROR("BUNDLE",
                  "Invalid frame dimensions: " + String(header_.frame_width) + "x" + String(header_.frame_height));
        return false;
    }
