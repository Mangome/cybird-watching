...
{
    char path[128];
    snprintf(path, sizeof(path), "/birds/%d/%d.bin", current_bird_.id,
             frame_index + 1);  // 从1开始编号，格式为1.bin, 2.bin等
...
bool BirdAnimation::tryManualImageLoad(const std::string& file_path)
{
    ...
    // LOG_DEBUG("ANIM", "Loading " + String(file_path.c_str()) + ", file_size: " + String(data_size) + ", free_heap: "
    // + String(free_heap)); // 注释掉以避免日志洪流
